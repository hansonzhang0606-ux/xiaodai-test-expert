...
> **版本**：v1.5.1 | **适用对象**：效贷测试专家管理员 | **更新日期**：2026-08-18
...
| 定时任务管理 | 维护每日 12:00 / 18:00 的 MySQL 数据同步自动化任务 |
...
│  [测试人员] 各自运行 init_mysql_config.py 生成本机 mysql_config.json           │
│      ↓                                                                     │
...
│      ↓                                                                     │
│  [测试人员] 运行 MySQL 初始化脚本（init_mysql_config.py）生成配置              │
│      ↓                                                                     │
...
| 节点 | 时机 | 测试人员做什么 | 涉及工具 |
|------|------|-------------|---------|
| 一次性安装 | 首次使用时 | 添加市场 → 安装套件 → 运行注册脚本 → 重启 → 运行 MySQL 初始化脚本 | WorkBuddy 界面、注册脚本、init_mysql_config.py |
...
3. 通知新人员按以下步骤安装：
   ...
   - **运行 MySQL 初始化脚本**：找到专家包 scripts 目录，运行 `python init_mysql_config.py --biz-line 效贷`，按提示输入管理员提供的 MySQL 密码，生成本机 `mysql_config.json`（路径：`~/.workbuddy/data/time-tracking/效贷/mysql_config.json`）

> 从 v1.5.0 起，工时数据通过 MySQL 共享数据库集中存储（取代 v1.4.x 的腾讯文档智能表格）。测试人员无需注册 GitHub 账号或生成 PAT，但必须运行 MySQL 初始化脚本生成本机配置，否则工时数据无法同步到团队共享数据库。
...
### 5.7 独立时间追踪 Skill（双宿主部署，v1.5.1 新增）

> **背景**：团队中测试人员的工具习惯不同——部分习惯使用 **WorkBuddy 测试专家（智能体）**，
> 部分习惯在 **VSCode / OpenCode 等 IDE 中直接使用 Skill 套件**。为同时适配两类场景，
> 已把「测试人员时间节省追踪」能力抽取为**独立通用 Skill**（`time-tracking-skill`，多业务线版），
> 两类场景共用同一套脚本，时间数据最终**汇聚到同一张 MySQL 表**。

#### 5.7.1 双宿主架构

```
┌─ 场景 A：WorkBuddy 测试专家 ─┐      ┌─ 场景 B：IDE Skill 套件 ──────┐
│  智能体形态（agent.md 驱动）  │      │  SKILL.md 驱动                │
│  报告用 present_files 展示    │      │  VSCode/OpenCode/Claude Code │
└──────────────┬───────────────┘      │  报告给文件路径，浏览器打开   │
               │                      └──────────────┬───────────────┘
               └────────────────┬────────────────────┘
                                ↓
                    本地 records.jsonl（每步记录，兜底）
                                ↓
            定时任务（12:00 / 18:00）幂等 upsert（sync_to_mysql.py）
                                ↓
             共享 MySQL 表 agent_time_tracking（按 biz_line_code 区分业务线）
```

| 场景 | 形态 | 驱动入口 | 报告展示 | 使用者 |
|------|------|---------|---------|--------|
| **A. WorkBuddy 测试专家** | 智能体（agent） | `agent.md` + 内嵌 skill | `present_files` 右侧面板预览 | 习惯 WorkBuddy 的测试人员 |
| **B. IDE Skill 套件** | 纯 Skill 包 | `SKILL.md`（VSCode/OpenCode/Claude Code 等可加载） | 回复中给文件路径，浏览器打开 | 习惯 IDE 的测试人员 |

**关键设计**：
- **脚本层完全宿主无关**：`record_time_saved.py` / `sync_to_mysql.py` / `init_mysql_config.py` / `sync_task.bat` 均为纯命令行，两类场景共用，无需重复开发
- **幂等键兼容**：`record_key = MD5(biz_line_code|employee|user_story|step_code|timestamp秒)`，同一张表内按业务线编码（XD/JWY/XR/XXD/ZHJ/AIJXC/ZHJLS）区分，两边数据并存不冲突
- **本机配置共用**：`mysql_config.json` 是本机私有配置（含密码），同一台电脑无论用哪种工具都共用一份
- **每台电脑只需初始化一次**：`python init_mysql_config.py --biz-line <业务线>`

#### 5.7.2 独立 Skill 的部署要点（管理员引导）

给 IDE 场景团队（如智慧记）分发 `time-tracking-skill.zip` 后，管理员需引导完成：

1. **解压嵌入**：解压到目标团队的 Skill 套件目录（`time-tracking-skill/`）
2. **配置业务线**：`config/time_tracking_config.yaml` 中设置 `default_biz_line: "<业务线名>"`（如"智慧记"），`storage_mode: "mysql"`
3. **花名册**：`config/team_roster.yaml` 已按 `biz_line_code` 数组格式预置多业务线成员，按团队实际人员维护
4. **MySQL 初始化**：每位测试人员本机运行一次 `python init_mysql_config.py --biz-line <业务线>`（密码由管理员单独告知）
5. **注册定时任务**：以管理员身份注册每日 12:00 / 18:00 的 `sync_task.bat` 定时任务（确认 `set BIZ_LINE=<业务线>` 已配置）

> ⚠️ 未初始化 `mysql_config.json` 时，时间数据**只保存在本机 JSONL**，不会同步到团队 MySQL；
> 记录功能本身不受影响（本地兜底），skill 会在会话启动时自动引导初始化。

#### 5.7.3 独立 Skill 与专家包的关系

- 独立 Skill 的脚本与专家包 `skills/ai-testcase-workflow-skill` 内的时间追踪脚本**同源**（v1.5.0 抽取 + 泛化）
- 独立 Skill 的 `prompts/time_tracking.md` 为 **v5 MySQL-only 口径**（移除腾讯文档 cloud 流程），并做了**宿主无关处理**（WorkBuddy 用 `present_files`，IDE 给文件路径）
- 专家包本身仍走 `agent.md` 驱动；两者数据格式、幂等键、目标表完全一致，可放心共存
- 产物：`time-tracking-skill.zip`（解压后根目录 `time-tracking-skill/`，含 scripts + config + prompts）
...
> 从 v1.5.0 起，`storage_mode` 改为 `mysql`（MySQL 共享数据库），数据在本地记录后由定时任务（12:00 / 18:00）幂等同步。
...
2. 确认电脑在 12:00 / 18:00 处于开机状态（Windows 计划任务依赖开机触发）
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v1.5.1 | 2026-08-18 | 新增 5.7 节「独立时间追踪 Skill（双宿主部署）」：说明 WorkBuddy 测试专家（智能体）与 IDE Skill 套件（VSCode/OpenCode/Claude Code）两类场景共用同一套时间追踪脚本，数据统一汇聚到 MySQL `agent_time_tracking` 表（按 biz_line_code 区分业务线）；记录独立 skill 的部署要点（default_biz_line / 花名册 / MySQL 初始化 / 定时任务）与宿主无关的展示方式 |
| v1.5.0 | 2026-08-18 | 工时数据存储从腾讯文档智能表格改为 MySQL 共享数据库（storage_mode 增加 `mysql`）；本地 JSONL 经每日定时任务（12:00/18:00）幂等 upsert 到 MySQL `agent_time_tracking` 表；新增 `init_mysql_config.py` 一键生成本机配置（含密码，不随专家包分发）；时间记录自动携带用户故事编号（user_story_code）；测试人员安装流程改为「添加市场 → 安装套件 → 注册脚本 → 重启 → 运行 MySQL 初始化脚本」5 步，**移除腾讯文档连接器步骤**（工时数据与报告展示均不再依赖腾讯文档，避免干扰测试人员）；同步更新两份指导手册，消除腾讯文档 vs MySQL 存储方式矛盾 |
