...
**这是已知现象**：WorkBuddy 从团队市场安装套件后，只注册为"套件"（显示在「我安装的」），不会自动注册为"专家"。
...
> 脚本原理：将专家 ID 写入 `~/.workbuddy/experts/custom/{userId}/experts.json`，这是 WorkBuddy "我的专家"列表的数据源。
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v1.3.6 | 2026-07-22 | 新增安装后注册脚本（register_expert.bat/ps1/sh），解决团队市场安装后专家不显示的问题；新增 Mac 版注册脚本；修复 Windows 上找不到 PowerShell 的回退逻辑 |
