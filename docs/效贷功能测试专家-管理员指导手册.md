...
> **版本**：v1.4.3 | **适用对象**：效贷测试专家管理员 | **更新日期**：2026-07-24
...
| **集成层** | 腾讯文档连接器（tencent-docs） | 工时数据实时同步到智能表格 + HTML 报告上传到【我的文档】 | 测试人员授权一次 |
| **数据层** | 腾讯文档智能表格 | 团队工时数据集中存储，所有测试人员数据汇总到同一表格 | 管理员初始化 |
...
│      ↓ AI 通过 tencent-docs skill 创建智能表格                                │
│  [腾讯文档智能表格] 创建完成，doc_id 回填到 time_tracking_config.yaml           │
│      ↓                                                                     │
│  [管理员] 编辑 team_roster.yaml 添加花名册成员 → 推送 GitHub                   │
│      ↓                                                                     │
...
│  [测试人员] 输入真实姓名                                                      │
│      ↓                                                                     │
│  [AI] Read 读取 team_roster.yaml → 精确匹配 active 成员                       │
...
│  │  [AI] 转发 CLOUD_SYNC_JSON → [腾讯文档智能表格] 实时写入     │             │
│  │  [AI] list_records 验证写入成功                               │             │
│  └──────────────────────────────────────────────────────────────┘             │
│      ↓ 不自动推进，等待测试人员指令                                            │
│  ┌─── 步骤② 需求评审 ─────────────────────────────────────────┐             │
│  │  [测试人员] "评审这份需求"                                    │             │
...
│      ↓                                                                     │
│  [AI] Read time_tracking_config.yaml → 确认 storage_mode=cloud               │
│      ↓                                                                     │
│  [腾讯文档 MCP] list_records → 读取全量数据 → 转为临时 JSON                    │
│      ↓                                                                     │
...
│  [管理员] 修改花名册 → 推送 GitHub → 通知测试人员更新套件+重跑注册脚本         │
...
| 智能表格初始化 | 首次部署时 | 在专家中说"初始化时间追踪表格"，AI 自动创建 | 腾讯文档 MCP、time_tracking_config.yaml |
...
| 查看全量统计 | 需要评估团队效能时 | 在专家中说"查看时间节省统计"，生成全业务线报告 | generate_time_analytics.py、腾讯文档智能表格 |
...
| 节点 | 时机 | 测试人员做什么 | 涉及工具 |
|------|------|-------------|---------|
| 一次性安装 | 首次使用时 | 添加市场 → 安装套件 → 运行注册脚本 → 重启 → 连接腾讯文档 | WorkBuddy 界面、注册脚本 |
