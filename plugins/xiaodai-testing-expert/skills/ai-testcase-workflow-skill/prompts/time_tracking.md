# 时间节省追踪规则（v5.3 — 强制反馈 + 二次确认 + 参考时间 + MySQL 共享数据库同步 + AI 自动初始化 + 花名册实时查 MySQL | 通用多业务线版）
...
> - **v1.6.0 变更**：专家从仅效贷扩展为效贷/小贷/效融三条业务线通用（工作流程一致）；保留「效贷测试专家」名称并泛化开场白；首次无配置时由测试人员选择业务线再初始化；知识库改为三业务线各自独立、严格按 biz_line 隔离。
