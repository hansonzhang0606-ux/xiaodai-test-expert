...
9. **查看统计必展示报告（强制，不可跳过）**：用户说"查看时间节省统计"/"查看时间统计"/"效能统计"等时，**必须同时完成三件事**——① 生成 HTML 报告文件；② 调用 `present_files` 工具在右侧面板打开预览；③ 在对话回复中附上报告文件的本地完整路径。三者缺一不可。禁止只播报数字而不展示报告。详见 `prompts/time_tracking.md` 第五节。
...
>
> **禁止只播报数字而不生成/展示报告。禁止生成报告但不调用 present_files。**
