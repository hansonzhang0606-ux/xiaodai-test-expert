...
3. 测试人员本机运行 `python scripts/init_mysql_config.py --biz-line 效贷` 生成 `mysql_config.json`，将 `storage_mode` 改为 `"mysql"`
