...
9. **查看统计必展示且必上传报告（强制，不可跳过，违者视为未完成）**：
   ...
   - **必须完成的 5 件事（缺一不可，必须在同一次回复中全部完成）**：
     1. **生成 HTML 报告文件**：调用 `scripts/generate_time_analytics.py` 生成。测试人员传 `--person "{姓名}"`，报告标题为"{姓名} 个人时间节省统计"，文件名 `time_analytics_效贷_{姓名}.html`；管理员不传 `--person`，标题为"效贷业务线时间节省统计"，文件名 `time_analytics_效贷.html`。
