...
> **v4.2 变更**：回复中同步告知本地 HTML 文件路径，以及腾讯文档中【更多】-【我的文件】-【任务成果】的具体导航位置。
...
> **重要**：触发后**不等于**只在聊天里展示表格/数字。触发后必须按下面完整流程执行：**读取数据 → 生成 HTML 报告 → 调用 present_files 展示 → 上传到腾讯文档【我的文档】（cloud 模式）→ 回复中给出本地路径和在线链接**。
...
> 1. **生成 HTML 报告文件**（`time_analytics_效贷.html`）
...
>
> **禁止行为**：禁止只以聊天表格/文字播报数字；禁止生成报告但不调用 `present_files`；禁止不上传腾讯文档或不给出链接；禁止用"我已在上方展示数据"等理由跳过文件生成与上传；禁止忽略腾讯文档导航路径提示。

**执行顺序**：

1. 先生成 HTML 报告文件
2. **立即**调用 `present_files` 工具展示报告（右侧面板打开预览）
3. **cloud 模式**：立即执行 aipage 上传工作流（见上方第 6 步）
4. 在最终回复中依次给出：
   - 本地完整路径（如 `C:\Users\kingdee\.workbuddy\data\time-tracking\效贷\time_analytics_效贷.html`）
...
11. **【强制】报告必展示且必上传（cloud 模式，未完成禁止回复）**：每次查看时间统计时，必须同时完成四件事——① 生成 HTML 报告；② 调用 `present_files` 工具在右侧面板打开预览；③ 在对话回复中附上报告本地完整路径；④ 将报告上传到腾讯文档【我的文档】并附上在线链接。四者缺一不可，缺少任意一项时**禁止发送最终回复**。禁止只以聊天表格/文字播报数字，禁止用"数据已在上方展示"等理由跳过文件生成与上传。
